...
def load_fmri_data(dataset, parcel_type, phenotypes_to_include, target_phenotype, categorical_features, n_subjects=None):
    """
    ...
    Args:
        ...
        n_subjects (int, optional): Number of subjects to randomly select. If None, use all subjects.
    ...
    for i, subject_id in enumerate(subject_ids):
        ...
        valid_fmri_data.append(fmri_data)
        valid_phenotypes.append(input_phenotypes[i])
        valid_targets.append(target_labels[i])

    total_valid_subjects = len(valid_subjects)
    print(f"Total valid subjects found: {total_valid_subjects}")
    
    # Randomly select n_subjects if specified
    if n_subjects is not None:
        if n_subjects > total_valid_subjects:
            print(f"Warning: Requested {n_subjects} subjects but only {total_valid_subjects} available.")
            n_subjects = total_valid_subjects
        else:
            # Randomly select indices from valid subjects
            selected_indices = np.random.choice(total_valid_subjects, size=n_subjects, replace=False)
            valid_fmri_data = [valid_fmri_data[i] for i in selected_indices]
            valid_phenotypes = [valid_phenotypes[i] for i in selected_indices]
            valid_targets = [valid_targets[i] for i in selected_indices]
            print(f"Randomly selected {n_subjects} subjects from {total_valid_subjects} valid subjects")
    
    # Create final tensors
    X = []
    y = []
    for fmri_data, phenotype_data, target in zip(valid_fmri_data, valid_phenotypes, valid_targets):
        fmri_tensor = torch.tensor(fmri_data, dtype=torch.float32)
        phenotype_tensor = torch.tensor(phenotype_data, dtype=torch.float32).repeat(fmri_tensor.shape[0], 1)
        combined_features = torch.cat((fmri_tensor, phenotype_tensor), dim=1)
        X.append(combined_features)
        y.append(target)

    print(f"Final sample size (number of subjects): {len(X)}")
...
def train_cycle(
    ...
    # Save to CSV
    csv_filename = f"QuixerTuning/{hyperparams['dataset']}/{hyperparams['parcel_type']}/classical_Small_{hyperparams['target']}_{hyperparams['lossfunction']}_{hyperparams['seed']}_{tuning_set}.csv"
