...
from QuixerfMRI_Regress import get_train_evaluate_regress
from QuixerfMRI_Class import get_train_evaluate_class

def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, default='UKB')
    parser.add_argument("--parcellation", type=str, default='HCP')
    parser.add_argument("--target", type=str, default='sex')
    parser.add_argument("--model-dir", type=str, default='UKB_sex_class.pt')
    parser.add_argument("--seed", type=int, default=2024)
    return parser.parse_args()
args=get_args()


batch_size = 16       # Example: 32 sequences per batch
...
target = args.target                    # sex, ADHD_label
...
def save_progress(completed_sets, progress_file=f"QuixerTuning/{args.dataset}/{args.parcellation}/{args.target}_{args.seed}_training_progress.json"):
    """Save the indices of completed hyperparameter sets"""
    with open(progress_file, 'w') as f:
        json.dump({'completed_sets': completed_sets}, f)

def load_progress(progress_file=f"QuixerTuning/{args.dataset}/{args.parcellation}/{args.target}_{args.seed}_training_progress.json"):
...
for idx, hyperparams in enumerate(hyperparameter_sets):
    ...
    try:
        ...
        train_evaluate_class = get_train_evaluate_class(device, set_label)
        
        # Train and evaluate for each hyperparameter set
        train_evaluate_class(hyperparams)
